...
const rotasCategorias = require('./routes/categoria.js');
const rotasProdutos = require('./routes/produtos');
const rotasPedidos = require('./routes/pedidos');
const rotasAuth = require('./routes/auth'); // 👈 ADICIONADO: Importação da rota de Login
...
app.use('/api/categoria', rotasCategorias);
app.use('/api/produtos', rotasProdutos);
app.use('/api/pedidos', rotasPedidos);
app.use('/api/auth', rotasAuth); // 👈 ADICIONADO: Registro da rota de Login
...
app.use((req, res, next) => {
    res.status(404).json({
        sucesso: false,
        mensagem: `Rota '${req.url}' não encontrada na API do Sabor Encantado.`
    });
});
...
app.listen(PORTA, () => {
    ...
    console.log(`   POST   /api/auth/cadastro`); // 👈 ADICIONADO na lista informativa do terminal
    console.log('');
